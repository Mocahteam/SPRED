...
int main (){
	...
	while (!PP_IsGameOver()){
		if (PP_GetNumUnits(ENEMY_COALITION) == 0){
			...
			for (i = 0 ; i < PP_GetNumUnits(MY_COALITION) ; i++){
				PP_Unit_GetPendingCommands(PP_GetUnitAt(MY_COALITION, i), &cmd);
				if (cmd.nbCmds == 0){
			...
			// ordonner aux unités de cibler l'unité ennemie la plus proche d'elle
			for (i = 0 ; i < PP_GetNumUnits(MY_COALITION) ; i++){
				u = PP_GetUnitAt(MY_COALITION, i);
				// enregistrer la cible s'il y en a une
				old = -1;
				PP_Unit_GetPendingCommands(u, &cmd);
				if (cmd.nbCmds > 0)
					if (cmd.cmd[0].code == ATTACK)
						if (cmd.cmd[0].nbParams == 1)
							old = cmd.cmd[0].param[0];

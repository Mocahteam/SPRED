...
local function Start ()
	...
	-- create my unit
	myUnits[1].id = Spring.CreateUnit("bit", 1120, Spring.GetGroundHeight(1120, 1760), 1760, "s", 0)
	myUnits[2].id = Spring.CreateUnit("byte", 1056, Spring.GetGroundHeight(1056, 1792), 1792, "s", 0)
...
local function Update (frameNumber)
	if frameNumber == 20 then
		...
		local msg
		if lang == "fr" then
			msg = "Point de ralliement des bytes\n(479, 1825)"
		else
	...
	if frameNumber == 80 then
		...
		local msg
		if lang == "fr" then
			msg = "Point de ralliement des bits\n(1400, 1371)"
		else

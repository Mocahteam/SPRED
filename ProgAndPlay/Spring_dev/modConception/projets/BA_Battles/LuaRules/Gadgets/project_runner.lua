...
--------------------------------------------------------------------------------
	...
	local unitName = Spring.GetModOptions()["basisunit"] -- get the basis unit
			...
			for i = 1,table.getn(teamList) do
				if teamList[i] ~= Spring.GetGaiaTeamID() then
					...
					if (level == "1" or level == "2") then
						...
						if isAI then
							-- add a unit
							local u = Spring.CreateUnit(unitName, startX, Spring.GetGroundHeight(startX, startZ), startZ, orientation, teamList[i])
							-- set FireState to "Return fire" == 1
							Spring.GiveOrderToUnit(u, CMD.FIRE_STATE, {1}, {});
						else
							-- add a fisrt unit
							local u = Spring.CreateUnit(unitName, startX, Spring.GetGroundHeight(startX, startZ-100), startZ-100, orientation, teamList[i])
							-- set FireState to "Return fire" == 1
							Spring.GiveOrderToUnit(u, CMD.FIRE_STATE, {1}, {});
							-- add a second unit
							u = Spring.CreateUnit(unitName, startX, Spring.GetGroundHeight(startX, startZ+100), startZ+100, orientation, teamList[i])
							-- set FireState to "Return fire" == 1
							Spring.GiveOrderToUnit(u, CMD.FIRE_STATE, {1}, {})
						end
					else
						local nbcol = 6 -- number of units columns
						local colSep = 150 -- separation between columns
						for col = 1,nbcol do
							-- compute x in order to center columns 1 and 2 on startX
							local x = startX+sens*((col-1.5)*colSep)
							local nbLine = 8 -- number of units lines
							for line = 1,nbLine do
								--local z = (lig - 1/2) * (Game.mapSizeZ / (nblig * 3))
								local lineSep = Game.mapSizeZ / (nbLine * 4) -- separation between lines
								local z = startZ+(line - (nbLine/2))*lineSep
								-- add a unit
								local u = Spring.CreateUnit(unitName, x, Spring.GetGroundHeight(x, z), z, orientation, teamList[i])
								-- set FireState to "Return fire" == 1
								Spring.GiveOrderToUnit(u, CMD.FIRE_STATE, {1}, {});
							end
						end

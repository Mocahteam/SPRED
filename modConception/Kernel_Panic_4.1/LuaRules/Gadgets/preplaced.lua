...
--SYNCED
	...
	function gadget:GameFrame(frame)
		...
		elseif frame==2 then
			if (tonumber(Spring.GetModOptions().preplaced) or 0)~=0 then

				-- Determine the minifac and special that will be placed for each team
				local ConcernedTeams={}
				...
				for _,g in ipairs(FreeSpots) do
					for _,t in ipairs(ConcernedTeams) do
				...
				local SpotsPerTeam=math.floor(#FreeSpots/#ConcernedTeams)
				...
				if SpotsPerTeam>0 then
					...
					for k=#FreeSpots,1,-1 do
						...
						if k>SpotsPerTeam*#ConcernedTeams then
							table.sort(
								FreeSpots,
								function(a,b)

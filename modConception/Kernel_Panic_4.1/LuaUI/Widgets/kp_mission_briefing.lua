...
local function IsAbove()
	if WG.KP_MissionInfoBox then
		local xMouse,yMouse = Spring.GetMouseState()
		if WG.KP_MissionInfoBox.Title and (xMouse > WG.KP_MissionInfoBox.Title.x1 and xMouse < WG.KP_MissionInfoBox.Title.x2 and yMouse > WG.KP_MissionInfoBox.Title.y1 and yMouse < WG.KP_MissionInfoBox.Title.y2) then
			return true
		elseif WG.KP_MissionInfoBox.Briefing and (xMouse > WG.KP_MissionInfoBox.Briefing.x1 and xMouse < WG.KP_MissionInfoBox.Briefing.x2 and yMouse > WG.KP_MissionInfoBox.Briefing.y1 and yMouse < WG.KP_MissionInfoBox.Briefing.y2) then
			return true
		else
			return false
		end
...
function widget:DrawScreenEffects(vsx,vsy)
	...
	if Spring.IsGUIHidden() then
		...
		WG.KP_MissionInfoBox=WG.KP_MissionInfoBox or {}
	end
	...
	local FontSize=WG.KP_MissionInfoBox.FontSize or math.max(13,4+vsy/100)
	local xSize, ySize = 0, 0
	local lines,maxWidth
	if WG.KP_MissionInfoBox.Title and WG.KP_MissionInfoBox.Title.lines then
		lines,maxWidth = WG.KP_MissionInfoBox.Title.lines, WG.KP_MissionInfoBox.Title.maxWidth
	else
	...
	WG.KP_MissionInfoBox.FontSize=FontSize
	WG.KP_MissionInfoBox.Title={x1=math.min(x1,x2),y1=math.min(y1,y2),x2=math.max(x1,x2),y2=math.max(y1,y2),xSize=math.abs(x2-x1),ySize=math.abs(y2-y1),lines=lines,maxWidth=maxWidth}
	...
	if IsAbove() then
		...
		xSize, ySize = 0, 0
		if WG.KP_MissionInfoBox.Briefing and WG.KP_MissionInfoBox.Briefing.lines then
			lines,maxWidth = WG.KP_MissionInfoBox.Briefing.lines, WG.KP_MissionInfoBox.Briefing.maxWidth
		else
			WG.KP_MissionInfoBox.Title.lines=nil
			lines,maxWidth = GetLines(GetMissionBriefing() or "No Intel")
		end
		xSize = FontSize*(1+maxWidth*TextWidthFixHack)
		ySize = FontSize*(1+#lines)

		-- Place briefing below title
		x1 = WG.KP_MissionInfoBox.Title.x1
		y2 = WG.KP_MissionInfoBox.Title.y1
		x2 = x1+xSize
		y1 = y2-ySize
		if WG.KP_MissionInfoBox.Title.x2 == MiniMap.x-3 then -- left of minimap
			x1 = MiniMap.x-xSize
			x2 = x1+xSize
		elseif WG.KP_MissionInfoBox.Title.y1 == MiniMap.y+MiniMap.sy+2 then -- above minimap
			y1=WG.KP_MissionInfoBox.Title.y2
			y2=y1+ySize
		end

		-- Saving the position of MissionInfoBox.briefing
		WG.KP_MissionInfoBox.Briefing={x1=math.min(x1,x2),y1=math.min(y1,y2),x2=math.max(x1,x2),y2=math.max(y1,y2),xSize=math.abs(x2-x1),ySize=math.abs(y2-y1),lines=lines,maxWidth=maxWidth}

		local r,g,b = 0.76,1,0
		gl.Blending(GL.SRC_ALPHA,GL.ONE_MINUS_SRC_ALPHA) -- default
		gl.Color(r,g,b,0.5)
		gl.Rect(x1,y1,x2,y2)
		gl.Color(r,g,b,1)
		...
		WG.KP_MissionInfoBox.Briefing=nil
	end
...
function widget:MouseWheel(up,value)
	if IsAbove() then
		if up then
			WG.KP_MissionInfoBox.FontSize = math.max(WG.KP_MissionInfoBox.FontSize - 1,2)
		else
			WG.KP_MissionInfoBox.FontSize = WG.KP_MissionInfoBox.FontSize + 1
		end
		return true
	end
	return false

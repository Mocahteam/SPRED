...
local BuildTestUnit=UnitDefNames.badblock.id
local DoomsSecond = nil -- Time before everything turns to block, in seconds
local Phase = 0 -- 0,1,2,3,4
local ActiveBlocks = {}
local BlockCountPerTeam = {}
--local FullBlocks = {}-- Not needed anymore: Progression uses ActiveBlocks and BlockTable, Unsynced get sent NewBlocks, and winner is decided from BlockCountPerTeam
	...
	if t then
		...
		if op and DoomsSecond then
			if Spring.GetGameSeconds()<DoomsSecond+op*t then
				...
				DoomsSecond=nil
				SendToUnsynced("SendColorWarsUpdate",0,"nil","")
...
function gadget:GameFrame(frame)
	...
	local NewBlocks={}-- To transmit to unsynced
		...
		for _,b in ipairs(ActiveBlocks) do
			if frame>=b.Birthday then
				...
				if bt1==2 and not bt2 then
					...
					table.insert(NewBlocks,("B%dT%dX%dZ%d"):format(b.BlockType,b.team,b.x,b.z))
					BlockCountPerTeam[b.team]=1+(BlockCountPerTeam[b.team] or 0)
					local north_growth=SeedBlock(b.BlockType,b.team,b.x,b.z-32,20)
					local south_growth=SeedBlock(b.BlockType,b.team,b.x,b.z+32,20)
					local west_growth=SeedBlock(b.BlockType,b.team,b.x-32,b.z,20)
					local east_growth=SeedBlock(b.BlockType,b.team,b.x+32,b.z,20)
					if west_growth or north_growth then
						SeedBlock(b.BlockType,b.team,b.x-32,b.z-32,30)
					end
					if west_growth or south_growth then
						SeedBlock(b.BlockType,b.team,b.x-32,b.z+32,30)
					end
					if east_growth or north_growth then
						SeedBlock(b.BlockType,b.team,b.x+32,b.z-32,30)
					end
					if east_growth or south_growth then
						SeedBlock(b.BlockType,b.team,b.x+32,b.z+32,30)
					end
					--end
				--else
	...
	-- This method was causing a memory overflow in Spring 100
	...
	-- So now using this much better method
	SendToUnsynced("SendColorWarsUpdate",Phase,tostring(DoomsSecond),table.concat(NewBlocks," "))
...
local function DrawBlocksVertices(blocks)
...
local function DrawBlocks(blocks)
	...
	gl.DepthTest(true)
	gl.DepthMask(true)
	gl.Texture(":a:bitmaps/kpsfx/GreyBlock.png")
	gl.BeginEnd(GL.QUADS,DrawBlocksVertices,blocks)
	gl.Texture(false)
	gl.DepthMask(false)
	gl.DepthTest(false)
	gl.Color(1,1,1,1)
...
function gadget:DrawWorldPreUnit()
	if DoomsSecond then
		if #NewBlocks>0 then
			--LimitedEcho("New blocks detected! #NewBlocks="..#NewBlocks)
			...
			local NewBlockDisplayList=gl.CreateList(DrawBlocks,NewBlocks)
			table.insert(BlockDisplayLists,NewBlockDisplayList)
			NewBlocks={}-- We can forget them, their visuals are stored in a display list now
		end
...
function gadget:DrawScreenEffects(vsx, vsy)
	...
	if DoomsSecond and not Spring.IsGUIHidden() then
		...
		if Phase>=2  then
			...
			for _,t in ipairs(Spring.GetTeamList()) do
				local count=BlockCountPerTeam[t]
				if count then
					CountDown.Show=true
					local CountTxt=count..(Phase<4 and "" or (count>=MaxBlockCount and " Winner!" or " Loser"))
					local tc={Spring.GetTeamColor(t)}
					gl.Color(tc[1],tc[2],tc[3],1)
			...
			CountDown.Show=true
			local t=math.floor(DoomsSecond-Spring.GetGameSeconds())
			local h=math.floor(t/3600)
			local m=math.floor((t%3600)/60)
			local s=t%60
...
local function ReceiveColorWarsUpdate(name,phase,doomssecond,newblocks)
	Phase=phase
	DoomsSecond=tonumber(doomssecond)
	for b,t,x,z in newblocks:gmatch("B(%d+)T(%d+)X([%-%.%d]+)Z([%-%.%d]+)") do
		local b,t,x,z=tonumber(b),tonumber(t),tonumber(x),tonumber(z)
		local y=Spring.GetGroundHeight(x,z)-- This uses unsynced heightmap, hopefully that's not noticeable
		table.insert(NewBlocks,{BlockType=b,team=t,x=x,y=y,z=z})
		BlockCountPerTeam[t]=(BlockCountPerTeam[t] or 0)+1
		MaxBlockCount=math.max(MaxBlockCount or 0,BlockCountPerTeam[t])
	end
end

function gadget:Initialize()
	gadgetHandler:AddSyncAction("SendColorWarsUpdate",ReceiveColorWarsUpdate)
end

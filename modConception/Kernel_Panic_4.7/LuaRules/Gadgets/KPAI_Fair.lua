...
local function DispatchCon(t,u,p)
	...
	if teamData[t].Lack.buildings<=0  then
		...
		-- Spreading cons to avoid clogging homebase exit :
		local pos=teamData[t].positions[p]
		local phase = math.pi*2*math.random()
		local amp = 128+128*math.random()
		Spring.GiveOrderToUnit(u,CMD.MOVE,{pos.x + math.cos(phase)*amp,0,pos.z + math.sin(phase)*amp},{})
		return
	end

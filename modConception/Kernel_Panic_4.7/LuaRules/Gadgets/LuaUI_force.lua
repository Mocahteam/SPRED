...
function gadget:Initialize()
	--Spring.Echo("Spring.GetConfigInt(\"LuaUI\")=="..tostring(Spring.GetConfigInt("LuaUI") or "nil").." Script.LuaUI('WriteAndRunState')=="..tostring(Script.LuaUI('WriteAndRunState') or "nil"))
	Force_LuaUI_timer = Spring.GetTimer()
	HasRanOnce = Spring.GetConfigInt(KPRanOnceKey)
	if HasRanOnce==3 then
		gadgetHandler:RemoveGadget()
		-- LuaUI=0 in springsettings stopped being read by the engine long ago
		-- gadget:Initialize run before widget:Initialize so the second bit doesn't work either

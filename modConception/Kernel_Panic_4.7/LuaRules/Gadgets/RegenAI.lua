...
--SYNCED
	...
	local function StoreIt(u,ud,x,z)
		...
		-- Because Kernel Panic building need datavents
		if isAnyBuilding[ud] then
			-- Hack to counter autospam widget
			local _,_,_,isAiTeam=Spring.GetTeamInfo(Spring.GetUnitTeam(u))
			if not isAiTeam then
				Spring.GiveOrderToUnit(u,CMD.REPEAT,{0},{})
			end
	...
	local function IsItOccupied(id)
		local x,_,z=Spring.GetUnitPosition(id)
		for _,u in ipairs(Spring.GetAllUnits()) do
			if UnitDefs[Spring.GetUnitDefID(u)].canMove==false then
				local ux,_,uz=Spring.GetUnitPosition(u)
				if math.abs(ux - x)<48 and math.abs(uz - z)<48 then
					return 1
				end
			end
		end
	...
	local function SaveRegenData(team)
		...
		if (string.lower(Spring.GetTeamLuaAI(team))==string.lower(gadget:GetInfo().name)) or
			...
			for _,u in ipairs(Spring.GetTeamUnits(team)) do
				...
				AddQueue(q,Spring.GetFactoryCommands(u,0xFFFF) or {})
				AddQueue(q,Spring.GetUnitCommands(u,0xFFFF) or {})
	...
	local function Regen(frame,uwoi,uwo,team)
		...
		elseif uwo.timer and uwo.status~=3 and uwo.status~=1 and uwo.status~=7 then
		...
		if uwo.status==0 then
			...
			if AlreadyFoundImpossible[udid] or (uwo.name=="packet" and AlreadyDispatching) then
				return
			end
			local BuilderID=nil
			...
			for _,b in ipairs(Spring.GetTeamUnits(team)) do
				...
				if bdbo and #bdbo>=1 then
					...
					if CanBuildIt and isUnitComplete(b) and
						#(Spring.GetUnitCommands(b,1) or {})==0 and
						#(Spring.GetCommandQueue(b,1) or {})==0 and
						#(Spring.GetFactoryCommands(b,1) or {})==0 and
		...
		if uwo.status==4 then
			...
			local q=Spring.GetCommandQueue(uwo.id,1)
			if (not q) or (#q==0) or q[1].id~=CMD.MOVE then -- if move finished
				...
				if uwo.name=="exploit" and UnitDefs[Spring.GetUnitDefID(uwo.id)].name=="bug" then
					Spring.GiveOrderToUnit(uwo.id,CMD.MOVE,{uwo.x,Spring.GetGroundHeight(uwo.x,uwo.z),uwo.z},{})
					Spring.GiveOrderToUnit(uwo.id,CMD_DEPLOY,{},{"shift"})
					RegenData[team][uwoi].status=7 -- the bug is queued into creating an exploit
					RegenData[team][uwoi].timer=frame
				else
				...
				if RegenData[team] then
					for _,u in ipairs(Spring.GetTeamUnitsByDefs(team,packet)) do
						if unwanted[u] then
							...
							local c = Spring.GetUnitCommands(u,1) or {}
							local stunned = Spring.GetUnitIsStunned(u)
							if #c==0 and (not enemy) and (not stunned) and vx*vx+vy*vy+vz*vz<=0.0001 then
								...
								if Spring.GetGameSeconds()>idleTimer[u]+maxIdleTime then
									...
									if closestID then
										--Spring.SendMessageToTeam(team,"Bufferising idle packet")
										Spring.GiveOrderToUnit(u,CMD_ENTER,{closestID},{"shift"})
									else
										--Spring.SendMessageToTeam(team,"No tele for idle packet")
									end
								end
							else
								if idleTimer[u] then
					...
					if frame==61 then
						...
						for uwoi,uwo in pairs(RegenData[team]) do
							if uwo.status~=6 then
								--Spring.SendMessageToTeam(team,uwo.name.." status is "..uwo.status)
	...
	function gadget:UnitDamaged(u,ud,team,damage,para,weapon,proj,attacker,aud,ateam)
		if damage>0.5 and RegenData[team] then
			if isTeleporter[ud] then
				...
				if attacker and Spring.ValidUnitID(attacker) and (not Spring.GetUnitIsDead(attacker)) and Spring.GetUnitSeparation(u,attacker)<=dispatchRange then
					local bufferSize=Spring.GetTeamRulesParam(Spring.GetUnitTeam(u),"bufferSize")
					if bufferSize and bufferSize>=minBuffer then
		...
		for _,rgteam in ipairs(Spring.GetTeamList()) do
			if RegenData[rgteam] then
				for uwoi,uwo in ipairs(RegenData[rgteam]) do
					if uwo.id==u then
						if atk==u and UnitDefs[ud].name=="bug" then
							--Spring.SendMessageToTeam(rgteam,"Bug morphed into exploit")
	...
	function gadget:UnitCreated(u,ud,team)
		if RegenData[team] then
			...
			for uwoi,uwo in ipairs(RegenData[team]) do
				if (uwo.name==UnitDefs[ud].name and (uwo.status==1 or uwo.status==7 or (uwo.status==0 and uwo.name=="packet")))
					or (uwo.status==1 and UnitDefs[ud].name=="bug" and uwo.name=="exploit")
					then
					local x,_,z=Spring.GetUnitPosition(u)
			...
			elseif Spring.GetGameFrame()>62 then
				if WannabeIdx then
					--Spring.SendMessageToTeam(team,u.." is not wanted because too far!")
				else
					--Spring.SendMessageToTeam(team,u.." is not wanted!")
				end
				unwanted[u]=true
			end
		end

...
-- Does not return anything, create three global functions:
--
-- + HSLfromRGB
-- Take Red,Green,Blue (each from 0 to 255, with 0,0,0 black and 255,255,255 white)
-- Return Hue,Saturation,Lightness (each from 0 to 255 as well)
--
-- + RGBfromHSL
-- Take Hue,Saturation,Lightness
-- Return Red,Green,Blue
--
-- + PickRandomColors
-- Take a list. Each element of the list must be a table.
-- Add to each of those element a new field, color, with three random numbers
-- The numbers are chosen in such a way that team colors are:
--   - Evenly hue-spaced
--   - Fully satured
--   - Randomly chosen lightness within 37% to 75%
--
-- Note: HSLfromRGB and RGBfromHSL both inputs and ouputs are in the 0..255 range, not 0..1
-- PickRandomColors instead adds color component in the 0..1 range
--
...
function PickRandomColors(TeamList)-- Add a color attribute to each element of TeamList
	...
	for p,_ in ipairs(TeamList) do
		...
		TeamList[RandOrder[p]].color={Red/255,Green/255,Blue/255}
	end
end

...
-- Does not return anything, create two global functions:
--
-- + GenerateSkirmish
--   with input arguments:
--   - Map: Must point to an entry in ModSpecific.MapsList, or at least be formatted the same
--   - Grouping: 0: Spectate, 1: Duel, 2: Team Game, 3: Challenge (1 human vs many AI), 4: Heroic (HOMF gamemode)
--   - Difficulty: 1 (easiest) to 4 (hardest)
--   - AlliesFaction: Must point to an entry in ModSpecific.FactionsList, or at least be formatted the same
--   - EnemiesFaction: Must point to an entry in ModSpecific.FactionsList, or at least be formatted the same
-- It returns a skirmish, as in, a table describing the startscript condition
--
-- + GenerateRandMapChoice
--  With no arguments
--  Return a map randomly selected from those allowed in ModSpecific
--  Note: A map here is not just a string, but an entry of ModSpecific.MapsList
...
local function GenerateRandInt(lower_bound,higher_bound)
...
function GenerateSkirmish(map,grouping,difficulty,AlliesFaction,EnemiesFaction,Handicaps)
	...
	skirm.ModOptions.nowalls=1
	skirm.ModOptions.nospecials=0
	skirm.ModOptions.ons=0
	skirm.ModOptions.sos=0
	skirm.ModOptions.colorwars=0
	skirm.ModOptions.homf=0
	skirm.ModOptions.preplaced=0
	skirm.EndCondition=1
	...
	elseif skirm.grouping==4 then -- Heroic
		skirm.ModOptions.homf=1
		skirm.ModOptions.preplaced=1
		preplaced={true,true,true}
		nbr_allies=1
		if skirm.difficulty==1 then -- Heroic Easy
			skirm.ModOptions.homf={["homf"]=1,["homf_rightclick"]=0}
			ons={2,0,0}
			nbr_enemies=1
			roster_string="Heroic Stroll"
		elseif skirm.difficulty==2 then -- Heroic Medium:
			skirm.ModOptions.homf={["homf"]=1,["homf_rightclick"]=0}
			ons={0,0,1}
			nbr_enemies=math.min(3,skirm.map.MaxPlayers-1)
			roster_string="Heroic Struggle"
		elseif skirm.difficulty==3 then -- Heroic Hard:
			ons={0,0,1}
			nbr_enemies=2
			roster_string="Heroic Feat"
		elseif skirm.difficulty==4 then -- Heroic Very Hard:
			nbr_enemies=math.min(3,skirm.map.MaxPlayers-1)
			ons={0,0,1}
			sos=true
			roster_string="Heroic Nightmare"
		end
	end
	...
	for p=0, nbr_allies+nbr_enemies-1 do
		...
		if skirm.players[1+p].bot then
			...
			if type(f)=="string" and f~="" and (not VFS or not VFS.FileExists(f)) then
		...
		if skirm.ModOptions.ons and skirm.ModOptions.ons~=0 then
		...
		-- Set handicap of each player
		-- Handicap is a weapon damage nerf / an armor boost to everybody else
		if Handicaps then
			if enemy then
				skirm.players[1+p].handicap=Handicaps.Enemies
			elseif p==player_num then
				skirm.players[1+p].handicap=Handicaps.Player
			else
				skirm.players[1+p].handicap=Handicaps.Allies
			end
		end

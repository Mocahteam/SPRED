...
function MakeDumpedUnitList(UnitList,UsedIDs,ArgAbbreviated,Grid)
	...
	for _,u in ipairs(SortedUL) do
		...
		--for _,o in ipairs(Spring.GetFactoryCommands(u,0xFFFF) or {}) do
		--	table.insert(OrdersList,o)
		--end
		for _,o in ipairs(Spring.GetCommandQueue(u,0xFFFF) or {}) do
			table.insert(OrdersList,o)
		end
		local build=false
...
function SpawnUnit(line_or_words,suggested_team) -- will return the unused words, the new ID, the former ID, or false
	...
	if udid and x and z then
		...
		local NewID=Spring.CreateUnit(udid,x,0,z,facing,team,buildProgress and true or false)
		if NewID then
...
function GenerateFromCurrentState(Abbreviated,Grid,OnlyStartScript,WithHeightMap)
	...
	script.ModOptions={}
	script.ModOptions.nowalls=nowalls and 1 or 0
	script.ModOptions.nospecials=nospecials and 1 or 0
	...
	script.EndCondition=Game.gameMode or tonumber(Spring.GetModOptions()["gamemode"])
	...
	-- Heroes of Mainframe mod options
	for _,homf_opt in ipairs(
		{"homf","homf_speed","homf_hp","homf_regen","homf_dmg","homf_range","homf_impulsefactor","homf_aoe","homf_hugground","homf_collidefriendly","homf_rightclick"}
		) do
		if Spring.GetModOptions()[homf_opt] then
			if not script.ModOptions.homf then
				script.ModOptions.homf={}
			end
			script.ModOptions.homf[homf_opt]=Spring.GetModOptions()[homf_opt]
		end
	end
	...
	for p=1, script.NbrPlayers do
		local teamNum,leader,isDead,isAiTeam,side,allyTeam,teamCustomOptions,incomeMultiplier = Spring.GetTeamInfo(TeamFromPlayer[p])
		script.players[p]={}
		script.players[p].allyteam=allyTeam
		script.players[p].handicap=math.floor(((incomeMultiplier or 1)-1)*100+0.5)
		...
		if p==player_num and script.isSpeccing==0 then
			...
			if not GotLuaAI or string.len(GotLuaAI)<2 then
				...
				local _,_,_,_,ShortName,_,_ =Spring.GetOldAIInfo(TeamFromPlayer[p])
				if ShortName and string.len(ShortName)>2 and ShortName~="UNKNOWN" and ShortName~="NullAI" and ShortName~="NullOOJavaAI" then
					script.players[p].aioverride=nil
					script.players[p].bot={
						ShortName=ShortName,
						LongName="It's a .dll/.so Bot!",
						PathAndFileName=""}
				else

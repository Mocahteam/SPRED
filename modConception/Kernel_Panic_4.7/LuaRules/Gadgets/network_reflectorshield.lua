...
function gadget:GameFrame(time)
	for i,d in pairs(damageList) do
		if Spring.ValidUnitID(d.unit) and not Spring.GetUnitIsDead(d.unit) then
			Spring.AddUnitDamage(d.unit, d.damage, 0, d.attacker, d.weapon)-- 94.0 requires a weapon ID or zero damage is dealt
		end
		damageList[i]=nil
	end
...
--function gadget:UnitPreDamaged(u, ud, team, damage, para, weapon, au, aud, ateam)-- For version prior to 94.0
function gadget:UnitPreDamaged(u, ud, team, damage, para, weapon, proj, au, aud, ateam)-- For 94.0 and beyond
	if damage>0.0001 and protected[u] and not para then
		if aud and not ateam then-- Fix to add retrocompatibility
			au, aud, ateam = proj, au, aud
		end
		if au and ateam ~= team and Spring.ValidUnitID(au) and not Spring.GetUnitIsDead(au) then
			table.insert(damageList,{unit = au, damage = damage*.5, attacker = u, weapon = weapon})
		end
		return damage*0.5
	end
end

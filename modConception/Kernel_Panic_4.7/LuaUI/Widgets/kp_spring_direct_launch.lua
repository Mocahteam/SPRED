...
local function DoTheRestart(startscriptfilename)
	...
	trimmed,_=string.gsub(trimmed,"([^\r])\n","%1\r\n")
	...
	--Spring.Echo(widget:GetInfo().name..": Nevermind, let's dump the script in a file instead of passing its content as string, and hope it works this time")
	local n_startscript=ModSpecific.ScriptFileName or "script.txt"
	local file=io.open(n_startscript,"wb")
	file:write(trimmed)
	file:flush()
	file:close()
	Spring.Echo(widget:GetInfo().name..": Ok, calling Spring.Restart(\""..n_startscript.."\",\"\") now!")
	Spring.Restart(n_startscript,"")
	Spring.Echo("Spring.Restart("..n_startscript..",\"\")")
	Spring.Echo(widget:GetInfo().name..": Just called Spring.Restart(\""..n_startscript.."\",\"\")")
	Spring.Echo(widget:GetInfo().name..": Wait, we shouldn't be here, should have restarted or crashed or quitted, for the second time, by now.")
end

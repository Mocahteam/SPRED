    ...
    if (i ~= "inherited") then
      ...
      if (t=="table")or(t=="metatable") then
        ...
        if (ot ~= "table")and(ot ~= "metatable") then
          Spring.Echo("Chili", "error", obj.name .. ": Wrong param type given to " .. i .. ": got " .. ot .. " expected table.")
          obj[i] = {}
        end
    ...
    if _internal then
      if self._hlinks and next(self._hlinks) then
        ...
        if hlinks_cnt > 1 or (v ~= self) then --// check if user called Dispose() directly
          Spring.Echo("Chili", "error", ("Tried to dispose \"%s\"! It's still referenced %i times!"):format(self.name, hlinks_cnt))
        end
      end
    end
...
function Object:AddChild(obj, dontUpdate)
  ...
  if (self.children[objDirect]) then
    Spring.Echo("Chili", "error", ("Tried to add multiple times \"%s\" to \"%s\"!"):format(obj.name, self.name))
    return
  end
...
function Object:HideChild(obj)
  ...
  if (not self.children[objDirect]) then
    --if (self.debug) then
      Spring.Echo("Chili", "error", "Tried to hide a non-child (".. (obj.name or "") ..")")
    --end
    return
  end
  ...
  if (self.children_hidden[objDirect]) then
    --if (self.debug) then
      Spring.Echo("Chili", "error", "Tried to hide the same child multiple times (".. (obj.name or "") ..")")
    --end
    return
  end
...
function Object:ShowChild(obj)
  ...
  if (not self.children_hidden[objDirect]) then
    --if (self.debug) then
      Spring.Echo("Chili", "error", "Tried to show a non-child (".. (obj.name or "") ..")")
    --end
    return
  end
  ...
  if (self.children[objDirect]) then
    --if (self.debug) then
      Spring.Echo("Chili", "error", "Tried to show the same child multiple times (".. (obj.name or "") ..")")
    --end
    return
  end

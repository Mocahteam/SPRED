...
/* visibles, à chaque groupe est affecté une cible qui se trouve être        */
/* l'unité adverse la plus proche. En cas de débordement, l'armée fuit vers  */
/* la zone de repli.                                                         */
/*****************************************************************************/
...
int main (){
	...
	distance = sens*PP_GetMapSize().x/8;
	...
	if (PP_GetNumUnits(ENEMY_COALITION) == 0){
		...
		while (!minePosee() && PP_GetNumUnits(ENEMY_COALITION) == 0)
			printf("Attendre depos des mines\n");

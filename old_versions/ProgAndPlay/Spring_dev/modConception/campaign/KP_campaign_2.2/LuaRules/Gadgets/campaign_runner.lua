...
local nbMissions = table.getn(campaign)+1 -- get the number of mission which make up the campaign (add 1 because [0] isn't counted)
	  ...
	  gameOver = campaign[startingMission][1].Update(Spring.GetGameFrame())
		...
		if gameOver == -1 or gameOver == 1 then
			...
			elseif gameOver == 1 then
				if startingMission == nbMissions-1 then
					_G.action.gameOver = "wonCampaign"
				else
					_G.action.gameOver = "won"
				end
			end
			if startingMission == 0 then
				_G.action.posMission = "first"
			elseif startingMission == nbMissions-1 then
...
local function DoTheRestart(startscript)
	...
	Spring.Echo(trimmed)
	Spring.Echo("\n\n<<END DUMPING TRIMMED>>\n\n")
	Spring.Echo("Ok, calling Spring.Restart(\""..params.."\",\"[GAME]{..}\") now!")
	Spring.Restart(params,trimmed)
	Spring.Echo("Just called Spring.Restart(\""..params.."\",\"[GAME]{..}\")")
...
function gadget:RecvLuaMsg(msg, player)
	if msg == "Previous mission" then
		DoTheRestart(VFS.LoadFile("Missions/KPC_DW/"..lang.."/"..campaign[startingMission-1][2]))
	elseif msg == "Replay mission" then
		DoTheRestart(VFS.LoadFile("Missions/KPC_DW/"..lang.."/"..campaign[startingMission][2]))
